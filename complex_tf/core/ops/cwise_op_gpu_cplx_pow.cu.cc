...
namespace pycuda {
  ...
  __global__ void CplxPowKernel(const complex<float> *in1,
	...
	out[i] = pow(in1[i], in2[i]);
      ...
      for (i = cta_start + tid; i < n; i += total_threads) {
	out[i] = pow(in2[0], in1[i]);
      }
    } else if (side == tensorflow::functor::right) {
      for (i = cta_start + tid; i < n; i += total_threads) {
	out[i] = pow(in1[i], in2[0]);
      }
    }
  }
} // namespace pycuda
...
namespace tensorflow {
  ...
  namespace functor {
	...
	pycuda::CplxPowKernel<<<config.block_count, config.thread_per_block, 0,
      ...
      void operator()(const GPUDevice& d,
		      ...
		      scalar_side side) {
	printf("\t\tCalling GPU kernel for CplxPow.\n");
	const int N = input1.size();
	CudaLaunchConfig config = GetCudaLaunchConfig(N, d);
	pycuda::CplxPowKernel<<<config.block_count, config.thread_per_block, 0,
	  d.stream()>>>((pycuda::complex<float> *)input1.data(),
    ...
    DEFINE_BINARY1(cplx_pow, complex64);
