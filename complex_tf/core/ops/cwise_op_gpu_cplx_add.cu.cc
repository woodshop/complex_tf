...
namespace pycuda {
  ...
  __global__ void CplxAddKernel(const complex<float> *in1,
...
namespace tensorflow {
  ...
  namespace functor {
	...
	printf("\t\tCalling GPU kernel for CplxAdd.\n");
	const int N = input1.size();
	CudaLaunchConfig config = GetCudaLaunchConfig(N, d);
	pycuda::CplxAddKernel<<<config.block_count, config.thread_per_block, 0,
    ...
    DEFINE_BINARY1(cplx_add, complex64);
