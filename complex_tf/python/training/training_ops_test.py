...
class TrainingOpsTest(TensorFlowTestCase):
  ...
  def testApplyGradientDescent(self):
    for (dtype, use_gpu) in itertools.product(
        [np.complex64], [True]):
      x = np.arange(100).astype(dtype) + np.arange(100, 0, -1).astype(dtype)
      alpha = np.array(2.0+0.0j).astype(dtype)
      delta = np.arange(100).astype(dtype) + 1j*np.arange(100, 0, -1).astype(dtype)
  ...
  def _testTypesForAdam(self, var, m, v, grad, force_gpu):
    self.setUp()
