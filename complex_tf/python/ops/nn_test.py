...
class CplxL2LossTest(tf.test.TestCase):
  ...
  def testGradient(self):
    # There is high error between the theoretical andnumerical gradient. Need to
    # investigate further. For now, we give this test high tolerance to error
    x_shape = [20, 7, 3]
   ...
   # import pdb; pdb.set_trace()
    with self.test_session(force_gpu=True):
      ...
      err = tf.test.compute_gradient_error(x, x_shape, output, [1], delta=.1)
    print("L2Loss gradient err = %g " % err)
    # err_tolerance is usually 1e-11
    err_tolerance = 1e-4
