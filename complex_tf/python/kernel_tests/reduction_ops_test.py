...
class MeanReductionTest(tf.test.TestCase):
  ...
  def _compare(self, x, reduction_axes, keep_dims, force_gpu=True):
    np_ans = x
    if reduction_axes is None:
  ...
  def _compareAll(self, x, reduction_axes):
    self._compare(x, reduction_axes, False, force_gpu=True)
    self._compare(x, reduction_axes, True, force_gpu=True)
  ...
  def testGradient(self):
    s = [2, 3, 4, 2]
    x = np.arange(1.0, 49.0).reshape(s).astype(np.complex64)
    with self.test_session():
      t = tf.convert_to_tensor(x)
  ...
  def testEmptyGradients(self):
    with self.test_session(force_gpu=True):
      x = tf.zeros([0, 3], dtype=tf.complex64)
