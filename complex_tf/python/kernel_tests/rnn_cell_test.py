...
from complex_tf.python.ops.rnn_cell import _linear as linear

class RNNCellTest(tf.test.TestCase):

  def testLinear(self):
    with self.test_session() as sess:
      # Using variable_scope initializer
      with tf.variable_scope(
          "root", dtype=tf.complex64,
      ...
      # Using custom matrix intializer
      with tf.variable_scope("new_root", dtype=tf.complex64):
        matrix_init = tf.Variable([[1-1j, 1-1j],[1-1j, 1-1j]],
                                  dtype=tf.complex64)
  ...
  def testCplxBasicRNNCell(self):
    with self.test_session() as sess:
      with tf.variable_scope("root",
                             initializer=tf.constant_initializer(0.5-0.5j),
                             dtype=tf.complex64):
        x = tf.zeros([1, 2], dtype=tf.complex64)
