...
class AssignOpTest(tf.test.TestCase):
  ...
  def testBasic(self):
    self._testTypes(np.arange(0, 20).reshape([4, 5]) +
                    1j*np.arange(20, 0, -1).reshape([4, 5]))

  ### The tests below are written for CPU and contain ops that maty not yet
  ### be available for GPU
  # def testAssignNonStrictShapeChecking(self):
  #   with self.test_session():
  #     data = tf.fill([1024, 1024], np.complex64(0+0j))
  #     p = tf.Variable([np.complex64(1.0+2.0j)])
  #     a = tf.assign(p, data, validate_shape=False)
  #     a.op.run()
  #     self.assertAllEqual(p.eval(), data.eval())

  #     # Assign to yet another shape
  #     data2 = tf.fill([10, 10], np.complex64(1.0+1.0j))
